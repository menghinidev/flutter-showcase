...
mixin _$PaginationState<T> {
  ...
  TResult when<TResult extends Object?>({
    required TResult Function(List<T> items, bool hasMorePages, int page) data,
  ...
  TResult? whenOrNull<TResult extends Object?>({
    TResult? Function(List<T> items, bool hasMorePages, int page)? data,
...
abstract class _$$DataImplCopyWith<T, $Res> {
  ...
  @useResult
  $Res call({List<T> items, bool hasMorePages, int page});
}
...
class _$DataImpl<T> implements Data<T> {
  const _$DataImpl(final List<T> items,
      {required this.hasMorePages, required this.page})
      : _items = items;
  ...
  int get hashCode => Object.hash(runtimeType,
      const DeepCollectionEquality().hash(_items), hasMorePages, page);
  ...
  TResult when<TResult extends Object?>({
    ...
    return data(items, hasMorePages, page);
  }
  ...
  TResult? whenOrNull<TResult extends Object?>({
    ...
    return data?.call(items, hasMorePages, page);
  }
...
abstract class Data<T> implements PaginationState<T> {
  const factory Data(final List<T> items,
      {required final bool hasMorePages,
      required final int page}) = _$DataImpl<T>;
...
abstract class Error<T> implements PaginationState<T> {
  const factory Error(final Object? e, [final StackTrace? stk]) =
      _$ErrorImpl<T>;
...
class _$OnGoingLoadingImpl<T> implements OnGoingLoading<T> {
...
abstract class OnGoingLoading<T> implements PaginationState<T> {
  const factory OnGoingLoading(final List<T> items) = _$OnGoingLoadingImpl<T>;
...
class _$OnGoingErrorImpl<T> implements OnGoingError<T> {
...
abstract class OnGoingError<T> implements PaginationState<T> {
  const factory OnGoingError(final List<T> items, final Object? e,
